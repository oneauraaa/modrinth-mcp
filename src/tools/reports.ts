...
export async function submitReport(args: {
  report_type: string; item_id: string; item_type: string; body: string;
}): Promise<unknown> {
  requireAuth();
...
export async function updateReport(args: { id: string; body?: string; closed?: boolean }): Promise<unknown> {
  requireAuth();
  const { id, ...patch } = args;
...
export function registerReportTools(server: McpServer): void {
  ...
  server.tool('modrinth_get_reports', 'Get all reports submitted by the authenticated user (requires MODRINTH_TOKEN)',
